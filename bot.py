...
# Path where login cookies are cached between searches (lives as long as the container is up)
COOKIE_FILE = "/tmp/paperfly_session.json"


def save_cookies(cookies: list) -> None:
    """Persist browser cookies to disk so the next search can skip login."""
    try:
        with open(COOKIE_FILE, "w") as f:
            json.dump(cookies, f)
        log.info(f"Session cookies saved ({len(cookies)} cookies)")
    except Exception as e:
        log.warning(f"Could not save cookies: {e}")


def load_cookies() -> list:
    """Load previously saved cookies, or return [] if none exist."""
    try:
        with open(COOKIE_FILE) as f:
            cookies = json.load(f)
        log.info(f"Loaded {len(cookies)} cached cookies")
        return cookies
    except Exception:
        return []
...
async def scrape_orders(phone: str, days_back: int, progress_cb=None) -> dict:
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # ── SESSION RESTORE (skip login if we have valid cached cookies) ──────
            cached_cookies = load_cookies()
            session_ok = False

            if cached_cookies:
                await progress("🍪 Trying cached session...")
                try:
                    await context.add_cookies(cached_cookies)
                    await page.goto(PAPERFLY_ORDER_URL)
                    await page.wait_for_load_state("networkidle")
                    await page.wait_for_timeout(2000)
                    if "/identity/login" not in page.url:
                        dbg(f"Session restored OK. URL: {page.url}")
                        await progress("✅ Session restored! Skipped login.")
                        session_ok = True
                    else:
                        dbg("Cached session expired — will re-login")
                        # Clear stale cookies from context
                        await context.clear_cookies()
                except Exception as ce:
                    dbg(f"Cookie restore failed: {ce} — will re-login")
                    try:
                        await context.clear_cookies()
                    except Exception:
                        pass
            ...
            if date_input_el:
                ...
                dbg("Date filled")
                # The MUI date-picker opens a calendar dialog when you type into it.
                # Press Escape to close it, then click somewhere neutral.
                await page.keyboard.press("Escape")
                await page.wait_for_timeout(400)
                await page.keyboard.press("Escape")  # double-tap in case first wasn't enough
                await page.wait_for_timeout(300)
                # Click somewhere safe (top-left corner) to blur the field
                await page.mouse.click(10, 10)
                await page.wait_for_timeout(300)
            else:
                dbg("No date input found — skipping date filter")

            # ── DISMISS ANY OPEN DIALOG (MUI date picker or announcement popup) ──
            # This is the critical step: the calendar dialog from the date picker
            # intercepts pointer events on the search button if not closed first.
            dbg("Checking for open dialogs before clicking search...")
            for _d in range(5):
                dialog = await page.query_selector("div.MuiDialog-root, div[role='dialog']")
                if not dialog:
                    dbg("No dialog found — safe to click")
                    break
                dbg(f"Dialog found (attempt {_d+1}), dismissing...")
                # Try close button inside dialog
                close_btn = await page.query_selector(
                    "div.MuiDialog-root button[aria-label='close'], "
                    "div[role='dialog'] button[aria-label='close'], "
                    "div.MuiDialog-root button.MuiIconButton-root"
                )
                if close_btn:
                    await close_btn.click()
                    dbg("Clicked dialog close button")
                else:
                    # Force-remove all dialogs from DOM via JavaScript
                    await page.evaluate("""() => {
                        document.querySelectorAll(
                            'div.MuiDialog-root, div.MuiModal-root, div[role="dialog"]'
                        ).forEach(el => el.remove());
                    }""")
                    dbg("Removed dialog via JS")
                await page.wait_for_timeout(500)
            ...
            try:
                # First try normal click (respects all checks)
                await search_btn.click(timeout=5000)
            except Exception as click_err:
                dbg(f"Normal click failed ({click_err}), using force=True...")
                # force=True bypasses pointer-event interception — the button is
                # still functional even if something visually overlays it.
